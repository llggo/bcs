...
func Compute(fillter []bson.M) ([]*Scan, error) {
	var res = []*Scan{}
	var err = ScanTable.C().Pipe(fillter).All(&res)
	return res, err
}

func CountScan(fillter bson.M) *Aggregate {
	var q = ScanTable.C().Find(fillter)
	c, err := q.Count()
	if err != nil {
		fmt.Print(err)
	}
...
func Count(where interface{}) (int, error) {
	return ScanTable.UnsafeCount(where)
}
...
func Compute2(filter bson.M, groupByField string) []*Aggregate {
	...
	var res = []*Aggregate{}
	var err = ScanTable.C().Pipe(pipeline).All(&res)
	if err != nil {
		fmt.Println(err)
	}
	return res

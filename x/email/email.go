...
func Send(addr, password, smtpserver string, port int, m *Message) error {
	auth := smtp.PlainAuth("", addr, password, smtpserver)
	return smtp.SendMail(smtpserver+":"+strconv.Itoa(port), auth, m.From.Address, m.Tolist(), m.Bytes())
}
...
func (e Email) SendMail() error {
	...
	err = Send(
		...
		e.Port,
		&Message{
			From:            mail.Address{Name: e.EmailName, Address: e.Address},
			To:              e.To,
			Cc:              e.Cc,
			Subject:         e.Subject,
			Body:            e.Body,
			BodyContentType: e.BodyContentType,
		})
	if err != nil {
		return err
	}
